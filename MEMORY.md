...
- **Web-push te telefoni (5 sht):** NDËRTUAR i tëri, i mbyllur (gated off), inert. Faza A `push_subscriptions`+RLS (live). Faza B `public/push-sw.js` push-only (pa fetch/claim, scope `/push-scope/` → s'prek freskinë), `lib/push.ts`, `PushOptIn` te /notifications; rojet e SW-së (layout.tsx + UpdatePrompt) RUAJNË `/push-sw.js`. Faza C: edge `send-push` DEPLOYED (verify_jwt=false, x-push-secret), `admin_settings.send_push_url` vënë, trigeri `tg_notification_web_push` APLIKUAR por gated (`app_config.push_enabled='false'`) — provë: inert.
  **5 SHT (v2) — VETË-PROVIZIONUES, PA DASHBOARD:** funksioni `send-push` gjeneron vetë çelësat VAPID në thirrjen e parë → `admin_settings.vapid_private/send_push_secret` (privat) + `app_config.vapid_public` (publik). Klienti e merr publik-un në runtime nga `/api/push/pubkey` (jo env Vercel). `push_enabled='true'`. Bootstrap u krye (provë: http 200, vapid_public 87 char, private+secret në DB). **MBETET VETËM:** pronari hap app-in (iOS: "Shto në ekran bazë") → /notifications → prek "Aktivizo" (gjest i detyruar nga shfletuesi, s'bëhet nga jashtë) → unë verifikoj rreshtin te push_subscriptions + provë dërgimi. Çelësat rrotullohen duke fshirë rreshtat te admin_settings (rigjenerohen vetë).
