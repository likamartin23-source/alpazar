...
export default function Home() {
  ...
  useEffect(() => { init() }, [])
  useEffect(() => { load() }, [cat, fil])

  async function init() {
    const [l, u] = await Promise.all([
      get('listings?select=id&is_active=eq.true'),
      get('profiles?select=id')
    ])
    setTotL(Array.isArray(l) ? l.length : 0)
    setTotU(Array.isArray(u) ? u.length : 0)
    await load()
  }

  async function load() {
    setLoading(true)
    let url = 'listings?select=id,title,price,currency,condition,city,is_premium,images,category_id&is_active=eq.true&order=is_premium.desc,created_at.desc&limit=10'
    if (cat !== 'all') {
      const cats = await get(`categories?slug=eq.${cat}&select=id`)
      if (Array.isArray(cats) && cats[0]) url += `&category_id=eq.${cats[0].id}`
    }
    if (fil === 'new') url += '&condition=eq.i_ri'
    if (fil === 'used') url += '&condition=eq.i_perdorur'
    if (fil === 'premium') url += '&is_premium=eq.true'
  ...
  async function search(e: any) {
    ...
    setLoading(true)
    const data = await get(`listings?select=id,title,price,currency,condition,city,is_premium,images&is_active=eq.true&title=ilike.*${encodeURIComponent(q)}*&limit=10`)
    setListings(Array.isArray(data) ? data : [])
    setLoading(false)
  }
  ...
  return (<>
    ...
    <div className="wrap">
      <div className="hdr">
        ...
        <form className="srch" onSubmit={search}>
          <div className="sbox"><i className="ti ti-search"/><input type="text" placeholder="Kërko çdo gjë në Shqipëri..." value={q} onChange={e=>setQ(e.target.value)}/></div>
          <button type="submit" className="sbtn">Kërko</button>
        </form>
        <div className="cgrid">
          {CATS.map(c=>(
            <button key={c.slug} className={`ci ${cat===c.slug?'on':''}`} onClick={()=>setCat(c.slug)}>
      ...
      <div className="body">
        ...
        <div className="frow">
          {[{id:'all',l:'Të gjitha'},{id:'new',l:'I ri'},{id:'used',l:'I përdorur'},{id:'premium',l:'Premium ⭐'}].map(f=>(
            <button key={f.id} className={`fb ${fil===f.id?'on':''}`} onClick={()=>setFil(f.id)}>{f.l}</button>
          ))}
        </div>
        <div className="sec"><span>🔥 Shpallje të fundit</span><a onClick={()=>setCat('all')}>Të gjitha →</a></div>
        <div className="lgrid">
          {loading ? <div className="lw"><div className="spin"/><div>Duke ngarkuar...</div></div>
          : listings.length===0 ? (
            <div className="empty">
              <i className="ti ti-mood-empty"/>
              <h3>Nuk ka shpallje aktualisht</h3>
              <p>Bëhu i pari që shton!<br/>Regjistrimi është falas.</p>
              <button className="ebtn" onClick={()=>go('/auth/login')}>+ Shto shpallje</button>
            </div>
          ) : listings.map((l:any)=>(
            <div key={l.id} className="lc" onClick={()=>go(`/listing/${l.id}`)}>
              ...
              <div className="lb">
                ...
                <div className="lm"><span className="ll"><i className="ti ti-map-pin" style={{fontSize:9}}/>{l.city||'Shqipëri'}</span></div>
              </div>
            </div>
          ))}
        </div>
        <div className="prem">
          ...
          <div className="prem-t"><strong>👑 Alpazar Premium — 9.99€/muaj</strong><span>Dyqan · Badge · Shpallje ∞ · Statistika</span></div>
