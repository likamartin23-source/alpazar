...
const CSS = `*{box-sizing:border-box;margin:0;padding:0;font-family:system-ui,sans-serif;} .wrap{display:flex;min-height:100vh;background:#f5f5f5;} .sb{width:180px;background:#111;display:flex;flex-direction:column;flex-shrink:0;} .sb-logo{padding:14px;border-bottom:0.5px solid #222;} .sb-logo .n{font-size:13px;font-weight:700;color:#F5C842;} .sb-logo .r{font-size:10px;color:#888;margin-top:2px;} .nl{padding:9px 14px;display:flex;align-items:center;gap:8px;text-decoration:none;color:#666;border-left:3px solid transparent;} .nl:hover{background:#1a1a1a;} .nl.on{background:#1a1a1a;border-left-color:#F5C842;color:#F5C842;} .nl i{font-size:15px;} .nl span{font-size:11px;} .nl.on span{font-weight:700;} .content{flex:1;padding:20px;} .ph{display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;} .pt{font-size:17px;font-weight:700;color:#111;} .lb{background:#1D9E75;color:#fff;border-radius:4px;padding:3px 9px;font-size:11px;font-weight:700;} .stats{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:16px;} .sc{background:#fff;border:0.5px solid #eee;border-radius:9px;padding:12px;} .sn{font-size:20px;font-weight:700;color:#111;} .sl{font-size:10px;color:#888;margin-top:2px;} .card{background:#fff;border:0.5px solid #eee;border-radius:9px;padding:16px;margin-bottom:14px;} .ct{font-size:13px;font-weight:700;color:#111;margin-bottom:10px;} table{width:100%;border-collapse:collapse;font-size:11px;} th{background:#f5f5f5;color:#888;font-weight:700;padding:7px 9px;text-align:left;border-bottom:0.5px solid #eee;font-size:10px;text-transform:uppercase;} td{padding:8px 9px;border-bottom:0.5px solid #f5f5f5;color:#333;vertical-align:middle;} tr:last-child td{border:none;} .sb_{border-radius:4px;padding:2px 6px;font-size:10px;font-weight:700;} .sa{background:#EAF3DE;color:#1D9E75;} .sp_{background:#FAEEDA;color:#BA7517;} .ss{background:#FFF0EE;color:#E63312;} .ab{border:none;border-radius:4px;padding:4px 8px;font-size:10px;font-weight:700;cursor:pointer;margin-right:3px;} .ba{background:#EAF3DE;color:#1D9E75;} .bp{background:#FAEEDA;color:#BA7517;} .bd{background:#FFF0EE;color:#E63312;} .tgl{width:26px;height:13px;border-radius:8px;position:relative;cursor:pointer;display:inline-block;} .ton{background:#1D9E75;} .tof{background:#ccc;} .td{width:9px;height:9px;background:#fff;border-radius:50%;position:absolute;top:2px;} .ton .td{right:2px;} .tof .td{left:2px;} .pm-r{background:#f9f9f9;border:0.5px solid #eee;border-radius:7px;padding:10px;margin-bottom:6px;display:flex;align-items:center;gap:10px;} .pm-inf{flex:1;} .pm-inf strong{font-size:11px;font-weight:700;color:#111;display:block;} .pm-inf span{font-size:10px;color:#888;} .cfg-r{display:flex;align-items:center;justify-content:space-between;padding:6px 0;border-bottom:0.5px solid #f0f0f0;} .cfg-r:last-child{border:none;} .cfg-r span{font-size:11px;color:#333;} input[type=text]{border:0.5px solid #ddd;border-radius:6px;padding:6px 10px;font-size:12px;width:100%;margin-bottom:8px;font-family:inherit;}`
...
export default function Admin() {
  ...
  const tabs = [['dash','layout-dashboard','Dashboard'],['payments','credit-card','Pagesat'],['methods','wallet','Metodat'],['settings','settings','Konfigurime']]
    ...
    <div className="wrap">
      <div className="sb">
        ...
        {tabs.map(([id,icon,label])=>(
          <a key={id} className={`nl ${tab===id?'on':''}`} onClick={()=>setTab(id as string)}>
      ...
      <div className="content">
        {loading ? <div style={{textAlign:'center',padding:40,color:'#888'}}>Duke ngarkuar...</div> : (<>
          {tab==='dash' && <>
            ...
            <div className="stats">
              <div className="sc"><div className="sn">{stats.users}</div><div className="sl">Perdorues</div></div>
              <div className="sc"><div className="sn">{stats.premium}</div><div className="sl">Premium aktiv</div></div>
              <div className="sc"><div className="sn">{stats.revenue.toFixed(2)}EUR</div><div className="sl">Te ardhura</div></div>
              <div className="sc"><div className="sn">{stats.listings}</div><div className="sl">Shpallje</div></div>
            </div>
            <div className="card"><div className="ct">Pagesat e fundit</div>
              <table><thead><tr><th>Perdoruesi</th><th>Plan</th><th>Shuma</th><th>Statusi</th><th>Veprime</th></tr></thead>
                <tbody>
                  {payments.length===0 ? <tr><td colSpan={5} style={{textAlign:'center',color:'#aaa',padding:20}}>Nuk ka pagesa</td></tr> :
                  payments.map((p:any)=>(
                    <tr key={p.id}>
                      <td>{p.profiles?.full_name||p.profiles?.username||'-'}</td>
                      <td>{p.plan==='monthly'?'Mujor':'Vjetor'}</td>
                      <td style={{fontWeight:700,color:'#1D9E75'}}>{p.amount_eur}EUR</td>
                      ...
                      <td>
                        ...
                        <button className="ab bd" onClick={()=>updateStatus(p.id,'cancelled')}>Anulo</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              {methods.map((m:any)=>(
                <div key={m.id} className="pm-r">
                  ...
                  <div style={{display:'flex',gap:6,alignItems:'center'}}>
                    <span className={`tgl ${m.is_active?'ton':'tof'}`} onClick={()=>toggleMethod(m.id,m.is_active)}><span className="td"/></span>
                    <button className="ab bd" onClick={async()=>{await supabase.from('payment_methods').delete().eq('id',m.id);fetchAll()}}>Fshi</button>
                  </div>
                </div>
              ))}
            </div>
            <div className="card"><div className="ct">Llogaria e adminit</div>
              {['IBAN bankar','PayPal email'].map(k=>(
                <div key={k}><label style={{fontSize:10,color:'#888',display:'block',marginBottom:3}}>{k}</label>
                <input type="text" placeholder={`Vendos ${k}...`}/></div>
              ))}
            </div>
          </>}
          {tab==='settings' && <>
            <div className="ph"><div className="pt">Konfigurimet</div></div>
            <div className="card"><div className="ct">Platforma</div>
              {settings.filter((s:any)=>['site_name','site_slogan','premium_monthly_price','premium_yearly_price','free_listings_limit'].includes(s.key)).map((s:any)=>(
                <div key={s.id}><label style={{fontSize:10,color:'#888',display:'block',marginBottom:3}}>{s.key.replace(/_/g,' ').toUpperCase()}</label>
                <input type="text" defaultValue={s.value} onBlur={async(e)=>{await supabase.from('admin_settings').update({value:e.target.value}).eq('id',s.id)}}/></div>
              ))}
            </div>
            <div className="card"><div className="ct">Vecorite</div>
