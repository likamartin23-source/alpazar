...
export default function ModerimiPage() {
    ...
    <div style={{ minHeight: '100dvh', background: 'var(--az-cream)', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '40px 20px' }}>
      <div style={{ width: '100%', maxWidth: 560 }}>
        <button type="button" aria-label="Kthehu" onClick={() => window.history.back()}
          style={{ background: 'none', border: 'none', color: '#555', cursor: 'pointer', fontSize: 'var(--fs-dysheme)', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 6 }}>
          ← Kthehu
        </button>
        <div style={{ background: '#fff', border: '1.5px solid #f0e6b0', borderRadius: 16, padding: 28 }}>
          {inner}
        </div>
        <p style={{ fontSize: 'var(--fs-dysheme)', color: '#555', textAlign: 'center', marginTop: 16, lineHeight: 1.6 }}>
  ...
  if (duke) return wrap(<div style={{ fontSize: 'var(--fs-dysheme)', color: '#555' }}>Duke ngarkuar…</div>)
  ...
  if (gabim) {
    ...
    return wrap(
      <>
        ...
        <div role="alert" style={{ background: '#FFF0EE', color: '#C42B0F', border: '1px solid #F09595', borderRadius: 8, padding: '10px 12px', fontSize: 'var(--fs-dysheme)' }}>{teksti}</div>
        {gabim === 'pa_autentikim' && (
          <a href="/auth/login" style={{ display: 'inline-block', marginTop: 12, background: '#111', color: '#fff', borderRadius: 10, padding: '11px 18px', fontSize: 'var(--fs-dysheme)', fontWeight: 700, textDecoration: 'none' }}>Hyr</a>
        )}
      </>
    )
  }
  ...
  return wrap(
    <>
      ...
      <div style={{ fontSize: 'var(--fs-dysheme)', color: '#555', marginBottom: 18 }}>
      ...
      <div style={{ background: '#FBF7E8', border: '1px solid #f0e6b0', borderRadius: 10, padding: 14, marginBottom: 18 }}>
        <div style={{ fontSize: 'var(--fs-dysheme)', fontWeight: 800, color: '#4A4A4A', textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: 6 }}>Arsyetimi</div>
        <div style={{ fontSize: 'var(--fs-dysheme)', color: '#111', lineHeight: 1.65 }}>
      ...
      {ank ? (
        <div style={{
          ...
          <div style={{ fontSize: 'var(--fs-dysheme)', fontWeight: 700, color: '#111', marginBottom: 6 }}>
            {ank.status === 'pending'  ? 'Ankimi yt është në shqyrtim'
             : ank.status === 'accepted' ? 'Ankimi u pranua — shpallja u rikthye'
             : 'Ankimi u refuzua'}
          </div>
          <div style={{ fontSize: 'var(--fs-dysheme)', color: '#555', lineHeight: 1.6 }}>
            Dorëzuar më {dt(ank.krijuar_me)}. „{ank.arsyeja}"
          </div>
          {ank.pergjigjja && (
            <div style={{ fontSize: 'var(--fs-dysheme)', color: '#111', marginTop: 10, lineHeight: 1.6 }}>
        ...
        <>
          <div style={{ fontSize: 'var(--fs-dysheme)', fontWeight: 700, color: '#111', marginBottom: 6 }}>Nuk je dakord?</div>
          <p style={{ fontSize: 'var(--fs-dysheme)', color: '#555', lineHeight: 1.6, marginBottom: 10 }}>
          ...
          <textarea
            ...
            style={inputStyle}
          />
          <div style={{ fontSize: 'var(--fs-dysheme)', color: arsyeja.trim().length >= MIN_ARSYE ? '#3B6D11' : '#555', marginTop: 4 }}>
            {arsyeja.trim().length}/{MIN_ARSYE} karaktere minimale
          </div>

          {dergon && dergon !== 'duke' && (
            <div role="alert" style={{ background: '#FFF0EE', color: '#C42B0F', border: '1px solid #F09595', borderRadius: 8, padding: '10px 12px', fontSize: 'var(--fs-dysheme)', marginTop: 10 }}>{dergon}</div>
          )}
          ...
          <button
            ...
            style={{ width: '100%', marginTop: 12, background: '#111', color: '#fff', border: 'none', borderRadius: 10, padding: 13, fontSize: 'var(--fs-dysheme)', fontWeight: 700, cursor: 'pointer', opacity: dergon === 'duke' ? 0.7 : 1 }}
          >
