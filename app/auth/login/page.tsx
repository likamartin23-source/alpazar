...
export default function Login() {
  ...
  .back{display:flex;align-items:center;justify-content:center;gap:4px;margin-top:16px;color:#888;font-size:12px;text-decoration:none;}`

  async function handle() {
    if (!email || !pass) { setMsg('err:Ploteso emailin dhe fjalekalimin!'); return }
      ...
      if (mode === 'login') {
        ...
        if (error) {
          if (error.message.includes('Email not confirmed')) {
            setMsg('info:Email nuk u konfirmua. Duke u regjistruar direkt...')
            // Provo me OTP pa email konfirmim
            const { error: e2 } = await supabase.auth.signInWithOtp({ email, options: { shouldCreateUser: false } })
            if (e2) setMsg(`err:${e2.message}`)
          } else {
            setMsg(`err:${error.message === 'Invalid login credentials' ? 'Email ose fjalekalim i gabuar!' : error.message}`)
          }
        } else {
          setMsg('ok:Hyrja u krye! Duke te ridrejtuar...')
        ...
        if (error) {
          ...
          // Provo login direkt pas regjistrimit
          const { error: loginErr } = await supabase.auth.signInWithPassword({ email, password: pass })
          if (!loginErr) {
            setMsg('ok:Llogaria u krijua! Duke te ridrejtuar...')
            setTimeout(() => window.location.href = '/', 800)
          } else {
            setMsg('ok:Llogaria u krijua! Kontrollo emailin per konfirmim ose provo te hysh.')
          }
        }
      }
    } catch(e: any) {
      setMsg(`err:${e.message || 'Ndodhi nje gabim. Provo perseri.'}`)
    }
    setLoading(false)
  }
  ...
  return (<>
    ...
    <div className="wrap">
      <div className="card">
        ...
        <a href="/" className="back">← Kthehu te faqja kryesore</a>
      </div>
    </div>
  </>)
}
