  ...
  return (
    <div className="card" style={{ borderColor: '#DDD6FE', background: '#FAF8FF' }}>
      <div className="row">
        ...
        <span style={{ background: active ? '#EDE9FE' : '#FFF8E1', color: active ? '#5B21B6' : '#856404', fontSize: 'var(--fs-dysheme)', fontWeight: 800, padding: '4px 9px', borderRadius: 999 }}>
...
export function PlansGrid({ plans, plan, sub, busy, act }: any) {
  ...
  return (
    <div className="card">
      ...
      <div className="grid">
        {same.map((p: any) => (
          <div key={p.id} className={`pcard ${plan?.id === p.id ? 'cur' : ''}`}>
            <div className="pname">
              {p.name}
              {p.tier === 'boost' && <span style={{ marginLeft: 5, fontSize: 'var(--fs-dysheme)', fontWeight: 800, color: '#7C3AED' }}>⚡</span>}
            </div>
            <div className="pprice">
              {L(p.price_all)} L <span className="muted">/ {p.months || 1} muaj</span>
            </div>
            <div className="muted" style={{ fontSize: 'var(--fs-dysheme)' }}>
      ...
      <div className="muted" style={{ fontSize: 'var(--fs-dysheme)', marginTop: 8 }}>
      ...
      {tier === 'premium' && (
        <div className="muted" style={{ fontSize: 'var(--fs-dysheme)', marginTop: 6 }}>
        ...
        <div key={i.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '.5px solid #f2f2f2' }}>
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 'var(--fs-dysheme)', fontWeight: 700, color: '#111' }}>
              {i.number}
              {i.file_kind === 'fiscal' && (
                <span style={{ marginLeft: 6, background: '#EAF3DE', color: '#3B6D11', fontSize: 'var(--fs-dysheme)', fontWeight: 800, padding: '2px 5px', borderRadius: 4 }}>TATIMORE</span>
              )}
            </div>
            <div className="muted" style={{ fontSize: 'var(--fs-dysheme)', marginTop: 2 }}>
              {i.plan_name}{i.issued_at ? ` · ${dateShort(i.issued_at)}` : ''}
            </div>
          </div>
          <div style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
            <div style={{ fontSize: 'var(--fs-dysheme)', fontWeight: 800, color: '#111' }}>
            ...
            {i.file_url
              ? <a href={i.file_url} target="_blank" rel="noopener noreferrer"
                  style={{ fontSize: 'var(--fs-dysheme)', color: '#C42B0F', fontWeight: 700 }}>Shkarko</a>
              : <span className="muted" style={{ fontSize: 'var(--fs-dysheme)' }}>
