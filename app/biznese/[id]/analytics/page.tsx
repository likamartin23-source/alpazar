  ...
  return (
    <svg viewBox={`0 0 ${Math.max(data.length, 1) * 10} 40`} style={{ width: '100%', height: 80, display: 'block' }} aria-hidden="true">
      {data.map((d, i) => {
        ...
        return <rect key={d.date} x={i * 10 + 1} y={40 - h} width={8} height={h} rx={2} fill="#E63312" opacity={0.85} />
      })}
    </svg>
  )
}
...
export default function BiznesAnalyticsPage() {
  ...
  if (loadError) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', gap: 12 }}>
      ...
      <button type="button" onClick={() => { setLoadError(false); changePeriod(period) }} style={{ background: '#F5C842', border: 'none', borderRadius: 24, padding: '10px 24px', fontWeight: 700, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' }}>Rifresko</button>
    </div>
  )
  ...
  return (
    <div style={{ maxWidth: 480, margin: '0 auto', padding: '0 0 80px', fontFamily: "'Plus Jakarta Sans',system-ui,sans-serif", background: '#f8f8f8', minHeight: '100vh' }}>
      ...
      <div className="topbar">
        ...
        <h1 style={{ fontWeight: 800, fontSize: 16, color: '#111', margin: 0 }}>Analitika e biznesit</h1>
      </div>
