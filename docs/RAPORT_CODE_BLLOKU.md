...
| FAZA 4 · kuota+skadimi | ✅ LIVE | migrim `20260822`,PR #84 → `133c482d` | DB additive live+provuar (demote_free_keep_newest mban 10 më të reja; grace T-7/-3/-1; riaktivizim idempotent i mbrojtur nga trigeri; EXECUTE hequr nga anon); UI /profile: numërues X/kufi, filtra Aktive/Të pauzuara/Të shitura, Riaktivizo; kufiri nga app_config; CI-green (sha256+2 riparime+tsc+teste+build, run #28 + PR); /api/health build=133c482d (anonim, db 209ms rt 127ms) |
| FAZA 5 · nën-paneli biznes | ✅ LIVE | PR #85 → `d48e1fd6` | tab Biznes: panel real 3-gjendjesh mbi `businesses` (jo premium→/premium; premium pa biznes→/biznese/new §1B; me biznes→"Vepro si"→/biznese/[id] + Edito); forma legacy shop_* e ruajtur (rietiketuar) për kokën e ngrirë (vendim pronari: additive); pa prekur koka/12 tabet/§1B/pagesat; CI-green (sha256+tsc+teste+build+E2E+Lighthouse, run #29 + PR); /api/health build=d48e1fd6 (anonim, db 445ms rt 149ms) |

| task#18 · SSR shitës/biznes | ✅ LIVE | PR #86 → `dd198312` | /listing/[id]: `initialSeller`/`initialSellerCount`/`initialBiz` nga serveri → blloku i shitësit + kontakti + karta e biznesit në HTML-in e parë (verifikuar anonim: shfaqet "Administratori Alpazar · Anëtar nga · 3 shpallje aktive · Shiko biznesin · bnvhvh"); anon key respekton RLS |
| §13 · SW self-heal | ✅ LIVE | PR #86 → `dd198312` | UpdatePrompt strikt opt-in: krahason NEXT_PUBLIC_BUILD_ID vs /api/health.build; banderolë e mbylllshme, rifreskon VETËM me klik; zero auto-reload (respekton doktrinën); pa timer-loop; /api/health build=dd198312 (anonim) |

**PËRFUNDIM BLLOKU:** FAZA 0 · 1 · 2 · 3 · 4 · 5 + task#18 (SSR) + §13 (self-heal) — të gjitha ✅ LIVE dhe të verifikuara anonimisht. Blloku i miratuar dhe polishimet §12/§13 janë të kompletuara.
