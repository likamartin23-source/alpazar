...
function AlpazarIcon() {
  return (
    <svg width="34" height="34" viewBox="0 0 40 40" fill="none">
      ...
      <path d="M27 29 Q27 27 30 27 Q33 27 33 29" stroke="#111" strokeWidth="1.5" fill="none" strokeLinecap="round"/>
      <line x1="28" y1="31" x2="32" y2="31" stroke="#111" strokeWidth="1" strokeLinecap="round"/>
      <line x1="28" y1="33" x2="31" y2="33" stroke="#111" strokeWidth="1" strokeLinecap="round"/>
    </svg>
  )
}
...
export default function Home() {
  ...
  async function fetchCounts() {
    const { count: lc } = await supabase
      .from('listings').select('*', { count: 'exact', head: true }).eq('is_active', true)
  ...
  const formatPrice = (price: number, cur: string) => {
    if (!price) return 'Çmim me marrëveshje'
    return cur === 'EUR'
    ...
    <>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .header{background:#F5C842;border-bottom:1px solid #e0b030;}
        ...
        .icon-btn{width:32px;height:32px;background:rgba(0,0,0,.10);border-radius:50%;display:flex;align-items:center;justify-content:center;border:none;cursor:pointer;position:relative;}
        .icon-btn i{font-size:16px;color:#111;}
        .notif-dot{position:absolute;top:-1px;right:-1px;width:12px;height:12px;background:#E63312;border-radius:50%;font-size:7px;color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;}
        ...
        .search-wrap{flex:1;background:#fff;border-radius:9px;display:flex;align-items:center;padding:0 12px;gap:8px;border:0.5px solid #ddd;}
        ...
        .cat-item{background:#fff;border:1px solid #e0b030;border-radius:8px;padding:7px 3px 5px;display:flex;flex-direction:column;align-items:center;gap:2px;cursor:pointer;border:none;font-family:inherit;transition:all .15s;}
        .cat-item.active{background:#F5C842;border:1px solid #F5C842;}
        .cat-item i{font-size:16px;color:#777;}
        .cat-item.active i{color:#111;}
        .cat-item span{font-size:7px;color:#555;text-align:center;font-weight:500;}
        .cat-item.active span{color:#111;font-weight:700;}
        .no-ads{background:#EAF3DE;border:0.5px solid #97C459;border-radius:6px;padding:5px 12px;display:flex;align-items:center;gap:6px;margin:0 10px 8px;}
        .no-ads i{font-size:13px;color:#3B6D11;}
        .no-ads span{font-size:9px;color:#3B6D11;font-weight:700;}
        .hero{background:#111;margin:0 10px 8px;border-radius:12px;padding:14px 16px;display:flex;align-items:center;justify-content:space-between;position:relative;overflow:hidden;}
        ...
        .ai-bar{margin:0 10px 8px;background:#111;border-radius:11px;padding:10px 14px;display:flex;align-items:center;gap:9px;border:1px solid #E63312;}
        ...
        .ai-btn{background:#E63312;color:#fff;border:none;border-radius:7px;padding:7px 12px;font-size:11px;font-weight:700;cursor:pointer;white-space:nowrap;font-family:inherit;}
        .sw{padding:0 10px;}
        .section-title{display:flex;justify-content:space-between;align-items:center;margin:8px 0 6px;}
        .section-title span{font-size:13px;font-weight:700;color:#111;display:flex;align-items:center;gap:4px;}
        .section-title a{color:#E63312;font-size:11px;font-weight:500;text-decoration:none;}
        ...
        .stories{display:flex;gap:9px;overflow-x:auto;padding-bottom:4px;margin-bottom:10px;}
        .stories::-webkit-scrollbar{display:none;}
        .story{min-width:52px;display:flex;flex-direction:column;align-items:center;gap:3px;}
        .story-ring{width:48px;height:48px;border-radius:50%;border:2.5px solid #E63312;display:flex;align-items:center;justify-content:center;font-size:20px;background:#fff;}
        .story-ring.add{border:2px dashed #F5C842;background:#FFFBEA;}
        .story-ring.add i{font-size:18px;color:#F5C842;}
        .story-name{font-size:8px;color:#888;text-align:center;max-width:52px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
        ...
        .badge-new{position:absolute;top:6px;left:6px;background:#E63312;color:#fff;font-size:9px;padding:2px 6px;border-radius:4px;font-weight:700;}
        .badge-used{position:absolute;top:6px;left:6px;background:#111;color:#F5C842;font-size:9px;padding:2px 6px;border-radius:4px;font-weight:700;}
        .badge-premium{position:absolute;top:6px;right:6px;background:#F5C842;color:#111;font-size:9px;padding:2px 6px;border-radius:4px;font-weight:700;}
        ...
        .empty-state{grid-column:1/-1;text-align:center;padding:30px 16px;background:#f9f5e0;border:0.5px solid #eee;border-radius:11px;}
        .empty-state i{font-size:36px;color:#e0b030;display:block;margin-bottom:8px;}
        .empty-state p{font-size:12px;color:#aaa;line-height:1.6;margin-bottom:10px;}
        .empty-cta{background:#F5C842;color:#111;border:none;border-radius:8px;padding:8px 16px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;}
        .premium-cta{margin-bottom:10px;background:#FFFBEA;border:1.5px solid #F5C842;border-radius:11px;padding:10px 14px;display:flex;align-items:center;gap:8px;}
        ...
        .prem-text strong{font-size:12px;font-weight:700;color:#111;display:block;}
        .prem-text span{font-size:9px;color:#888;}
        .prem-btn{background:#111;color:#F5C842;border:none;border-radius:7px;padding:7px 12px;font-size:11px;font-weight:700;cursor:pointer;white-space:nowrap;font-family:inherit;}
        .bottom-nav{position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:480px;background:#111;padding:9px 10px 14px;display:flex;justify-content:space-around;align-items:center;border-top:1px solid #222;z-index:100;}
        ...
        .spinner{display:inline-block;width:28px;height:28px;border:3px solid #F5C842;border-top-color:#E63312;border-radius:50%;animation:spin .7s linear infinite;margin-bottom:8px;}
      ...
      <div className="wrap">
        ...
        <div className="header">
          ...
          {/* KATALOG KATEGORISH — UNIFORM 4x4 */}
        ...
        <div className="hero">
          ...
          <div>
            ...
            <p>Platforma #1 shqiptare<br />e tregtisë dhe shpalljeve online</p>
          </div>
        ...
        <div className="sw">
          ...
          <div className="trust-row">
            <div className="trust-card tc-green">
              ...
              <span>Shitës të verifikuar</span>
            </div>
          ...
          <div className="stories">
            <div className="story">
              <div className="story-ring add"><i className="ti ti-plus" /></div>
              <span className="story-name">Shto</span>
            </div>
            <div className="story">
              <div className="story-ring">📱</div>
              <span className="story-name">Tech</span>
            </div>
            <div className="story">
              <div className="story-ring">🚗</div>
              <span className="story-name">Auto</span>
            </div>
            <div className="story">
              <div className="story-ring">🏠</div>
              <span className="story-name">Prona</span>
            </div>
            <div className="story">
              <div className="story-ring">🔧</div>
              <span className="story-name">Shërbime</span>
            </div>
            <div className="story">
              <div className="story-ring">🐄</div>
              <span className="story-name">Kafshë</span>
            </div>
          </div>
          ...
          <div className="section-title">
            <span>
              <i className="ti ti-flame" style={{ color: '#E63312', fontSize: 16 }} />
              Shpallje të fundit
            </span>
            <a href="#">Të gjitha →</a>
          </div>
            ...
            <div className="listings-grid">
              {listings.length === 0 ? (
                <div className="empty-state">
                  ...
                  <p>Nuk ka shpallje aktualisht.<br />Bëhu i pari që shton!</p>
                  <button className="empty-cta" onClick={() => window.location.href='/listing/new'}>
                    + Shto shpallje
                  </button>
                </div>
              ) : (
          ...
          <div className="premium-cta">
            ...
            <div className="prem-text">
              ...
              <span>Dyqan · Badge · Shpallje ∞ · Statistika · 9.99€/muaj</span>
            </div>
            <button className="prem-btn" onClick={() => window.location.href='/premium'}>
              Shiko →
            </button>
          </div>
        </div>
        ...
        <nav className="bottom-nav">
          ...
          <button className="nav-item" onClick={() => window.location.href='/search'}>
            <i className="ti ti-search" /><span>Kërko</span>
          </button>
          <div className="nav-add" onClick={() => window.location.href='/listing/new'}>
            <i className="ti ti-plus" />
          </div>
          <button className="nav-item" onClick={() => window.location.href='/messages'}>
